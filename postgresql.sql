...
SELECT order_id,order_amount FROM orders WHERE order_amount BETWEEN 50 AND 100;
SELECT order_id,order_amount FROM orders WHERE order_amount IS  NULL ;
SELECT order_id,order_amount FROM orders WHERE order_amount IS Not  NULL ;

-- Fetch  and Limit Fitering data operation
SELECT * FROM customer
OFFSET 10
FETCH FIRST 5 ROWS ONLY; -- skip first 10 rows add show the five rows
SELECT * FROM customer
ORDER BY customer_id
LIMIT 10 OFFSET 5; -- skip first 5 rows and show the 10 rows
